 ...
 */
public class DisableSpiConfigLoader implements IConfigLoader {
	private static ILogger log = LoggerFactory.getLogger(DisableSpiConfigLoader.class);
	
	@Override
	public void load(String arguments) throws Exception {
		boolean disableAll = CoreConstants.CONFIG_KEY_DISABLE.equals(Config.getKv(CoreConstants.SPI_PLUGIN_KEY, CoreConstants.CONFIG_KEY_ENABLE));
		List<SPI> disableSpi = InstanceManage.getSpis().stream().filter(spi -> {
			...
			return (disableAll && !CoreConstants.CONFIG_KEY_ENABLE.equals(enableConfig))
					|| CoreConstants.CONFIG_KEY_DISABLE.equals(enableConfig);
