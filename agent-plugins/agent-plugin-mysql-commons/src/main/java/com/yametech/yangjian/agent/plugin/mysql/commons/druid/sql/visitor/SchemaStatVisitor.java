...
import com.yametech.yangjian.agent.plugin.mysql.commons.druid.sql.dialect.mysql.ast.expr.MySqlExpr;
import com.yametech.yangjian.agent.plugin.mysql.commons.druid.sql.dialect.mysql.visitor.MySqlASTVisitorAdapter;
import com.yametech.yangjian.agent.plugin.mysql.commons.druid.sql.repository.SchemaObject;
import com.yametech.yangjian.agent.plugin.mysql.commons.druid.sql.repository.SchemaRepository;
    ...
    public boolean visit(SQLOrderBy x) {
        ...
        if (query != null) {
            for (SQLSelectOrderByItem item : x.getItems()) {
                ...
                } else if (expr instanceof MySqlExpr) {
                    continue;
                }
            }
        }
        ...
        if (createStmt != null
            ...
            for (SQLTableElement e : createStmt.getTableElementList()) {
                if (e instanceof SQLColumnDefinition) {
                    ...
                    TableStat.Column column = addColumn(tableName.toString(), columnName.toString());
            ...
            TableStat.Column column = addColumn(expr.toString(), "*");
            if (isParentSelectItem(x.getParent())) {
                column.setSelec(true);
            }
        }
