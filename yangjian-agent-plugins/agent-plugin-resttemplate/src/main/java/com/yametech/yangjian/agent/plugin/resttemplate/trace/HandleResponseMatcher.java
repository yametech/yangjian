 ...
 * @date 2020/6/26
 */
public class HandleResponseMatcher implements ITraceMatcher {

    @Override
    public IConfigMatch match() {
        return new CombineAndMatch(Arrays.asList(
                new ClassMatch("org.springframework.web.client.RestTemplate"),
                new MethodNameMatch("handleResponse")
        ));
    }

    @Override
    public LoadClassKey loadClass(MethodType type, MethodDefined methodDefined) {
        return new LoadClassKey("com.yametech.yangjian.agent.plugin.resttemplate.trace.HandleResponseInterceptor");
    }
